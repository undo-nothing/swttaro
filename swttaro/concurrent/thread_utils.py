...
def thread_run(fn, arg_list=None, kwarg_list=None, pool_size=10, callback=None, return_result=None):
    '''
    线程运行过程不允许ctrl-c中断
    '''
    ...
    with ThreadPoolExecutor(pool_size) as pool:
        ...
        for arg, kwarg in zip(arg_list, kwarg_list):
            task = pool.submit(fn, *arg, **kwarg)
            if callback:
                task.add_done_callback(callback)
            handle_list.append(task)

        return [i.result() for i in handle_list] if return_result else handle_list
